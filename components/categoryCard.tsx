...
const CategoryCard = ({ category, lang }: { category: Category, lang: 'fr' | 'es' | 'en' }) => {
    ...
    return (
        <Card className="border max-w-[280px] w-[80vw] border-gray-200 bg-white rounded-xl overflow-hidden h-full hover:border-blue-300 transition-all duration-200 group/v1">
            <CardContent className="p-0 flex flex-col h-full">
                <div className="h-52 w-full overflow-hidden relative">
                    <img
                        src={category.images[0]}
                        alt={category.title[lang]}
                        className="h-full w-full object-cover group-hover/v1:scale-105 transition-transform duration-500"
                    />
