...
export function CategoriesSection({ categories }: { categories: Array<Category> }) {
    ...
    const swiperItems = categories.map((category, index) => (
        <Card key={index} className="border border-gray-200 rounded-xl overflow-hidden cursor-pointer h-full hover:border-blue-300 hover:shadow-lg transition-all duration-200 group">
